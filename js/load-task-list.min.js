$(document).ready(function(){let openDataLink='https://data.austintexas.gov/resource/x6vs-siqw.json';let openData;$.ajax({url:openDataLink,type:"GET",data:{"$limit":6000,"$$app_token":"n89g7s9iUyusfWDEHDHMtGcVT"}}).done(function(data){let nameArray=_.chain(data).pluck('assigneduser').uniq().value();$("#inspectorID").autocomplete({source:nameArray});openData=data;});function dateFormatting(datestring){let _d=new Date(datestring);let yr=_d.getFullYear();let mth=_d.getMonth()+1;let day=_d.getDate();if(isNaN(yr)){return'';}else{return mth+"/"+day+"/"+yr;}}
