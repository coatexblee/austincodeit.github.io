$(document).ready(function(){let openDataLink='https://data.austintexas.gov/resource/x6vs-siqw.json';let openData;$.ajax({url:openDataLink,type:"GET",data:{"$limit":6000,"$$app_token":"n89g7s9iUyusfWDEHDHMtGcVT"}}).done(function(data){var nameArray=_.chain(data).pluck('assigneduser').uniq().value();$("#inspectorID").autocomplete({source:nameArray});openData=data;});function dateFormatting(datestring){let _d=new Date(datestring);let yr=_d.getFullYear();let mth=_d.getMonth();let day=_d.getDate();if(isNaN(yr)){return'';}else{return mth+"/"+day+"/"+yr;}}
...
let filteredData=_.filter(openData,function(row){return row.assigneduser==chosenName;})
$(filteredData).each(function(i){$("#availableAddressRows").append('<tr>'+
...
'</tr>');activeElement=$("#routableAddressRows > tr:not(.placeholder):last-child").children("td#location");global_func.validateRemoveButton();global_func.adjustRowCount();global_func.placeAddressOnMap(newAddress,popUpText,false);});};});