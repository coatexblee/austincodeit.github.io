let global_pdf={};let global_func={};let $activeElement;$(document).ready(function(){let map,directionsService,directionsDisplay,geocoder,addressMarkerArray=[],iconCount=0;let labels='0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ';let initialize=function(){if(typeof google!=='object'){setTimeout(function(){initialize();},1000)}
geocoder=new google.maps.Geocoder();directionsService=new google.maps.DirectionsService;directionsDisplay=new google.maps.DirectionsRenderer({draggable:false});let myLatlng=new google.maps.LatLng(30.3344316,-97.6791038);let mapOptions={zoom:14,center:myLatlng,mapTypeId:google.maps.MapTypeId.ROADMAP}
map=new google.maps.Map(document.getElementById('map'),mapOptions);google.maps.event.addDomListener(window,"resize",function(){let center=map.getCenter();google.maps.event.trigger(map,"resize");map.setCenter(center);});directionsDisplay.addListener('directions_changed',function(){console.log('now hwat?');});}
...
let newMarker=new google.maps.Marker({position:location,label:labelObject,map:map,draggable:false})
