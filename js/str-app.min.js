var map;$(document).ready(function(){let openDataLink='https://data.austintexas.gov/resource/673d-jv2y.json';$.ajax({url:openDataLink,type:"GET",data:{"$limit":2000,"$$app_token":"n89g7s9iUyusfWDEHDHMtGcVT"}}).done(function(data){let strTypeCount=_.countBy(data,'str_type');let zipCodeCount=_.countBy(data,'prop_zip');let addressCount=_.countBy(data,'prop_address');let strTypeValues=_.values(strTypeCount);let strTypeKeys=_.keys(strTypeCount);let zipValues=_.values(zipCodeCount);let zipKeys=_.keys(zipCodeCount);let addressKeys=_.keys(addressCount);$("#full-count").html(data.length);let bscCount=0,citeCount=0,inspectCount=0,novCount=0,warningCount=0,nullCount=0;let activityTypeChart=Highcharts.chart('container-type',{chart:{type:'bar'},title:{text:'STRs by Type'},xAxis:{categories:strTypeKeys},yAxis:{title:{text:''}},series:[{name:['Total Count'],data:strTypeValues,dataLabels:{enabled:true,align:'right',style:{}}}]});let zipCountChart=Highcharts.chart('container-zip',{chart:{type:'column'},title:{text:'STRs by Zip Code'},xAxis:{categories:zipKeys},yAxis:{title:{text:''}},series:[{name:['Total Count'],data:zipValues,dataLabels:{enabled:true,align:'right',style:{}}}]});_api_key='AIzaSyBbnhwYIXT-MBTVIaDxS9kzbqIOmoeqcRU'
geocoder=new google.maps.Geocoder();function addMarker(location,popUpText){var cityCircle=new google.maps.Circle({strokeColor:'#FF0000',strokeOpacity:0.8,strokeWeight:2,fillColor:'#FF0000',fillOpacity:0.35,map:map,center:location,radius:200});var infoWindow=new google.maps.InfoWindow({content:popUpText});google.maps.event.addListener(cityCircle,'click',function(ev){infoWindow.setPosition(ev.latLng);infoWindow.open(map);});}
console.log('1');let addressCleaned=addressKeys.map(function(elem){return elem.substring(9,elem.length).trim()+", Austin, TX";});console.log('starting');let i=0;let secondTry=[];let secondTryKeys=[];function addressLoop(){geocoder.geocode({'address':addressCleaned[i]},function(results,status){console.log(addressCleaned[i]);if(status==google.maps.GeocoderStatus.OK){console.log(addressCleaned[i]+" "+addressKeys[i]);addMarker(results[0].geometry.location,addressKeys[i]);if(i<addressCleaned.length){setTimeout(addressLoop,500);i=i+1;}}else if(status==google.maps.GeocoderStatus.OVER_QUERY_LIMIT){console.log('pause.... at '+i+' position...');setTimeout(addressLoop,2500);}else{console.log('err');}});}
addressLoop();});var initialize=function(){if(google==null){initialize();}
var myLatlng=new google.maps.LatLng(30.2764099,-97.7507724);var mapOptions={zoom:13,center:myLatlng,mapTypeId:google.maps.MapTypeId.ROADMAP}
